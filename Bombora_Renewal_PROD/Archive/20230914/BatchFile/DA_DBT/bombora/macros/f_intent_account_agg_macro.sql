...
{%- if table_exists -%}
    ...
    (
    SELECT distinct
    concat(trim("domain"),'_',trim(cluster_initial),'_',datestamp) as f_intent_account_agg_key, 
    md5(trim(concat(trim("domain"),'_',trim(cluster_initial),'_',trim(accountid)))) as "nq_external_id",
    accountid,accountname,"domain",company,cluster_ui "cluster",cluster_type_ui "cluster_type",datestamp,count(topic) topic_count,round(avg(composite_score::integer)) avg_surge_score,
    concat(string_agg(concat(replace(replace(topic,',','-'),'^',','),' ',':',' ',composite_score),', '),',') topic_data
    FROM {{ ref(table1) }}  group by accountid,accountname,"domain",company,cluster_ui,cluster_type_ui,cluster_initial,datestamp) a
    left outer join
...
{%- else -%}    
    ...
    FROM {{ ref(table1) }}  group by accountid,accountname,"domain",company,cluster_ui,cluster_type_ui,cluster_initial,datestamp
{%- endif -%}
)
select *
