...
modelDir:str = "/home/ubuntu/appdata/Bombora_Renewal_PROD/BatchFile/DA_DBT/bombora/models"
with open("/home/ubuntu/appdata/Bombora_Renewal_PROD/BatchFile/multitenant.json") as tenantfile:
    ...
    for elem in json_settings:
          for key, value in elem.items():
            if key=='db_schema' and value != 'bombora_dwh':
              ...
              with open("/home/ubuntu/appdata/Bombora_Renewal_PROD/BatchFile/DA_DBT/src/kpi.json") as file:
                  ...
                  for key, value in w.items():
                    ...
                    if key== 'model':
                      ...
                      if  history >0 and posthook== 'y' :
                        ...
                        modelString:str = f'''
                        {{{{ config(materialized='{materialized}',unique_key='{uniquekey}', schema='{folder_name}', alias ='{model}', post_hook=["drop index if exists {folder_name}.ind_domain1","create index if not exists ind_domain1 on {folder_name}.{model}(trim(trailing '/' from REGEXP_REPLACE(REGEXP_REPLACE(REGEXP_REPLACE('domain','https://',''),'http://',''),'www.','')))"]) }}}}                      
                        {dependency}
                        {{{{ {macro}(custom_schema='{folder_name}',table1='{param}_{folder_name}') }}}}
                        '''
                        with open(f"{path}/{model}_{folder_name}.sql", "w") as f:
