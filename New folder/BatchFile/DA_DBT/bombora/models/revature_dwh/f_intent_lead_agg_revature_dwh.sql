                        ...
                        {{ config(materialized='incremental',unique_key='f_intent_lead_agg_key', schema='revature_dwh', alias ='f_intent_lead_agg', post_hook=["delete from revature_dwh.f_intent_lead_agg where datestamp < (select max(datestamp) -84 from revature_dwh.f_intent_lead_agg) " ,"update revature_dwh.loadexecution_detail set loadenddatetime =current_date, loadcomplete = 'y' where loadcomplete is null"]) }}                      
