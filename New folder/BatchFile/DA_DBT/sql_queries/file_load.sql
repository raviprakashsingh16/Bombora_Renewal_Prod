...
begin
select COALESCE(to_char(max(datestamp) + 7,'yyyymmdd'),'20230604') into v_datestamp  from "bombora_dwh".f_adat_bombora;
select COALESCE(max(datestamp) + 7,to_date('20230604','yyyymmdd')) into v_next_datestamp  from "bombora_dwh".f_adat_bombora;
